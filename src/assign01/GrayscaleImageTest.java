...
class GrayscaleImageTest {
    ...
    void testExtremeValues() {
        var extremeImage = new GrayscaleImage(new double[][]{{0, 255}, {255, 0}});
        assertEquals(0, extremeImage.getPixel(0, 0));
        assertEquals(255, extremeImage.getPixel(1, 0));
        assertEquals(255, extremeImage.getPixel(0, 1));
        assertEquals(0, extremeImage.getPixel(1, 1));
    }

    @Test
    void testUniformBrightness() {
        var uniformImage = new GrayscaleImage(new double[][]{{50, 50}, {50, 50}});
        assertEquals(50, uniformImage.averageBrightness());
    }
}