 ...
 * Author: Ben Jones and Shawn Zhang
 */
public class GrayscaleImage {
    ...
    public double getPixel(int x, int y){
        ...
        if(x < 0 || y < 0 || y >= imageData.length || x >= imageData.length){
            throw new IllegalArgumentException("Coordinates outside image bounds");
        }
    ...
    public GrayscaleImage normalized(){
        ...
        double[][] normalizedValue = new double[imageData.length][imageData[0].length];

        //Apply scale factor
        for (int row = 0; row < imageData.length; row++) {
            for (int col = 0; col < imageData[row].length; col++) {
                normalizedValue[row][col] = imageData[row][col] * scaleFactor;
            }
        }

        return new GrayscaleImage(normalizedValue);
    }
    ...
    public GrayscaleImage mirrored(){
        ...
        double[][] mirroredData = new double[rows][cols];

        for (int row = 0; row < imageData.length; row++) {
            for (int col = 0; col < imageData[row].length; col++) {
                int mirroredCol = cols - 1 - col;
                mirroredData[row][mirroredCol] = imageData[row][col];
            }
        }
     ...
     * @param width width of cropped square
     * @param height height of cropped square
    ...
    public GrayscaleImage cropped(int startRow, int startCol, int width, int height){
        if (startRow < 0 || startCol < 0 || width <= 0 || height <= 0
        || startRow + height > imageData.length || startCol + width > imageData[0].length){
            throw new IllegalArgumentException("Invalid cropping value");
        }
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                croppedData[i][j] = imageData[startRow + i][startCol + j];
            }
        }
    ...
    public GrayscaleImage squarified(){
        ...
        int squareSize = Math.min (originalHeight, originalWidth);

        double[][] square = new double[squareSize][squareSize];

        //offset calculation
        int rowOffset = (originalHeight - squareSize) / 2;
        int colOffset = (originalWidth - squareSize) / 2;

        //fill
        for (int i = 0; i < squareSize; i++) {
            for (int j = 0; j < squareSize; j++) {
                square[i][j] = imageData[i + rowOffset][j + colOffset];
            }
        }

        return new GrayscaleImage(square);
    }
}
