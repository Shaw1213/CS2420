 ...
 * 
 * @author Erin Parker & Shawn Zhang
 * @version March 14, 2024
 */
public class GraphUtility {

	/**
	 * To check if there is a path between the item.
	 *
	...
	public static <Type> boolean areConnected(List<Type> sources, List<Type> destinations, Type srcData, Type dstData)
		...
		if(!(sources.contains(srcData) || destinations.contains(srcData)) || !(sources.contains(dstData) || destinations.contains(dstData))) {
			throw new IllegalArgumentException("it does not contain the data");
		}

		// Create a graph with the given sources and destinations.
	...
	public static <Type> List<Type> shortestPath(List<Type> sources, List<Type> destinations, Type srcData, Type dstData)
		...
		Graph<Type> graph = new Graph<>(sources, destinations);
		// Calculate the shortest path between the source and destination.
	...
	/**
	 * To sort the sources and destinations together.
	 *
	 * @param sources, destinations
	 * @return  a list of the sorted sources and destinations
	 */
